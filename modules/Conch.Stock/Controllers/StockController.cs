...
namespace Conch.Stock.Controllers
{
    ...
    public class StockController : ControllerBase
    {
        ...
        public async Task<BaseResult> UpdateStockBlockGoodsNum([FromState("StockGoodsBlockNum", "Id")] StateEntry<UpdateNumInput> input)
        {
            bool saveFailed;
            try
            {
                ...
                {
                    saveFailed = false;
                    var result = await _stockDBContext.StockGoods.Where(c => input.Value.List.Select(d => d.Id).Contains(c.GoodsId)).ToListAsync();
                    if (result == null || result.Count < 1)
                    {
                        return new BaseResult().IsError("商品不存在");
                    }

                    foreach (var goods in input.Value.List)
                    {
                        // 查找商品库存
                    ...
                    try
                    {
                        if (!await input.TrySaveAsync())
                        {
                            return new BaseResult().IsError("ETag锁定商品库存失败");
                        }
